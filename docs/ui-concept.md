...
WANGAI should let a player choose one app, check whether audio and translation are ready, then start or stop listening. The desktop window is a compact, single-page control surface. The active session and F8 action are visible first. History opens in a side sheet; settings opens in a second side sheet with advanced controls disclosed only when needed. The main view does not repeat transcript history or expose server model configuration.
...
| Settings sheet | Change or clear source | Diagnostics, capture tuning, hotkeys, Overlay and updates in disclosures |
| History sheet | Inspect translations from this session | Opens over the control surface |
...
- [Apple popovers](https://developer.apple.com/design/human-interface-guidelines/popovers/): use transient UI for a few related tasks and avoid cascading popovers. WANGAI uses a larger scrollable side sheet for its many settings rather than stacking tiny popovers.
...
- [SteelSeries GG Sonar mixer release notes](https://techblog.steelseries.com/2023/07/25/GG-notes-43.0.0.html): the mixer gained a contextual master settings panel and direct shortcut access. WANGAI keeps source and listening controls in view and moves adjustments into a contextual sheet.
