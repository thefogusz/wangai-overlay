...
export function SettingsApp({ activeTab, advancedSection = "audio" }: { activeTab: SettingsTab; advancedSection?: AdvancedSection }) {
  ...
  return <main className="settings-app min-h-screen px-6 py-5">
    <header className="settings-header"><a className="settings-brand" href={settingsHref("overview")}><span className="settings-brand-key">W</span><span>WANGAI<small>LIVE TRANSLATION</small></span></a><nav aria-label="เมนูหลัก" className="settings-main-nav"><a aria-current={activeTab === "overview" ? "page" : undefined} href={settingsHref("overview")}>หน้าหลัก</a><a aria-current={activeTab === "history" ? "page" : undefined} href={settingsHref("history")}>ประวัติ</a><a aria-current={activeTab === "advanced" ? "page" : undefined} href={advancedHref("audio")}>ตั้งค่า</a></nav>{isDesktop() && <button className="settings-quit" onClick={() => void api.quitApp().catch((error) => setToast({ kind: "error", text: errorText(error) }))}><Power className="size-4" />ออกจากโปรแกรม</button>}</header>
    ...
    {activeTab === "advanced" && <>
      <nav aria-label="การตั้งค่าขั้นสูง" className="mb-5 flex flex-wrap gap-2 rounded-2xl border border-white/10 bg-[#1d1f25] p-2"><a aria-current={advancedSection === "audio" ? "page" : undefined} className={button} href={advancedHref("audio")}><AudioLines />เสียงและแอป</a><a aria-current={advancedSection === "ai" ? "page" : undefined} className={button} href={advancedHref("ai")}><Cloud />AI และคำศัพท์</a><a aria-current={advancedSection === "controls" ? "page" : undefined} className={button} href={advancedHref("controls")}><SlidersHorizontal />ปุ่มลัดและ Overlay</a><a className={`${button} ml-auto`} href={settingsHref("overview")}>กลับหน้าหลัก</a></nav>
