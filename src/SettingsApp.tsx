...
import { Cpu, History, KeyRound, LoaderCircle, Power, RefreshCw, Save, Settings2, SlidersHorizontal, TriangleAlert, Volume2 } from "lucide-react";
...
export function SettingsApp({ activeTab, advancedSection = "audio" }: { activeTab: SettingsTab; advancedSection?: AdvancedSection }) {
  ...
  const audioHelpStatus = !settings.listeningSource ? "ยังไม่ได้เลือกแอป" : !runtime.listening ? "ยังไม่ได้เริ่มใช้งาน" : !runtime.attachedSource ? "หาแอปที่เลือกไม่พบ" : runtime.audioLastSeenAtMs == null ? "ยังไม่มีเสียงเข้ามา" : (runtime.audioPeakDbfs ?? -96) <= -90 ? "ได้รับข้อมูลเสียง แต่เสียงยังเงียบ" : "ได้รับเสียงจากแอปแล้ว";
  ...
  return <main className={`settings-app settings-one-page ${utility ? "settings-utility" : "settings-control"}`}>
    ...
    {activeTab === "advanced" && <div className="utility-sections">
      <section className="space-y-4">
        <details className="settings-diagnostics"><summary><Volume2 />แก้ปัญหาเสียง<span>เปิดเมื่อไม่ได้ยินคำแปล</span></summary><div className="settings-audio-help">
          <p className="settings-audio-help-status">สถานะตอนนี้: <strong>{audioHelpStatus}</strong></p>
          {runtime.captureWarning && <p className="settings-audio-help-warning"><TriangleAlert aria-hidden="true" />{runtime.captureWarning}</p>}
          <ol><li>เปิดเสียงพูดในแอปที่เลือก แล้วตรวจว่าแอปยังเปิดอยู่</li><li>ตรวจ Volume Mixer ของ Windows ว่าแอปส่งเสียงไปยังอุปกรณ์ที่ใช้อยู่</li><li>หากเลือกผิด ให้กลับหน้าหลักแล้วกด “เปลี่ยน” ที่แหล่งเสียง</li></ol>
