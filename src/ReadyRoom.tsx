import { AudioLines, Check, ChevronRight, Cloud, Globe2, Headphones, LoaderCircle, MessageSquareText, Radio, ShieldCheck, TriangleAlert } from "lucide-react";
...
export function ReadyRoom({ settings, runtime, history, busy, previewMode, onToggleListening, onOpenSourcePicker, onOpenWebCompanion, webCompanionOrigin, webRuntime, notification }: Props) {
  ...
  const needsAttention = Boolean(runtime.lastError || runtime.captureWarning) || !runtime.workerReady || ["offline", "degraded"].includes(runtime.aiService.state);
  ...
  return <div className="ready-room-grid">
    ...
    <section className="ready-room-panel" aria-labelledby="ready-room-title">
      <div className="ready-room-intro"><div><p className="eyebrow">WANGAI LIVE</p><h2 id="ready-room-title">{needsSource ? "เลือกแอปแล้วเริ่มแปล" : runtime.listening ? "กำลังฟังและแปล" : "พร้อมเริ่มแปล"}</h2><p>คำแปลจะขึ้นบน Overlay ระหว่างที่คุณเล่นเกม</p></div><span className={`ready-summary ${needsSource || needsAttention ? "is-warning" : runtime.listening ? "is-live" : "is-ready"}`}>{needsSource || needsAttention ? <TriangleAlert /> : runtime.listening ? <AudioLines /> : <Check />}{needsSource ? "ต้องตั้งค่า" : needsAttention ? "ต้องตรวจสอบ" : runtime.listening ? "กำลังทำงาน" : "พร้อมใช้งาน"}</span></div>
      <div className="ready-primary-action"><span className="ready-game-mark" aria-hidden="true"><Headphones /></span><div className="ready-primary-context"><small>{needsSource ? "เริ่มจากตรงนี้" : runtime.listening ? "แอปที่กำลังฟัง" : "แอปที่เลือก"}</small><strong>{settings.listeningSource?.displayName ?? "เลือกเกมหรือแอปที่จะฟัง"}</strong><span>{runtime.listening ? "ฟังเสียงอยู่ · แปลเป็นไทยแบบสด" : configured ? "พร้อมรับเสียงและแสดงคำแปล" : needsSource ? "เลือกเกม Discord หรือเบราว์เซอร์" : "รอระบบเสียงหรือบริการ AI พร้อม"}</span></div>{needsSource ? <button aria-label="เลือกแอปที่จะฟัง" className="ready-listen-button is-select" onClick={onOpenSourcePicker}><Headphones /><span>เลือกแอป</span><small>ขั้นตอนแรก</small></button> : <button className={`ready-listen-button ${runtime.listening ? "is-listening" : ""}`} disabled={busy === "listen" || previewMode || (!runtime.listening && !configured)} onClick={onToggleListening}>{busy === "listen" ? <LoaderCircle className="animate-spin" /> : runtime.listening ? <AudioLines /> : <Headphones />}<span>{runtime.listening ? "หยุดฟัง · F8" : "เริ่มฟัง · F8"}</span><small>{runtime.listening ? "หยุดการแปลเสียง" : "เปิด Overlay"}</small></button>}{!configured && !needsSource && <div className="ready-action-hint"><strong>{!runtime.workerReady ? "กำลังเตรียมระบบเสียง" : "กำลังเชื่อมต่อบริการแปล"}</strong><span>{runtime.lastError ?? "สถานะจะอัปเดตอัตโนมัติเมื่อพร้อม"}</span></div>}</div>
      <div className="ready-source-list">
        <Row action={needsSource ? undefined : "เปลี่ยน"} icon={<Radio />} index={1} meterLabel="ระดับเสียงขาเข้า" meterValue={runtime.audioPeakDbfs} name={settings.listeningSource?.displayName ?? "ยังไม่ได้เลือกแอป"} onAction={onOpenSourcePicker} readiness={incoming} title="แหล่งเสียงที่ฟัง" />
...
function Row({ index, icon, title, name, readiness, meterLabel, meterValue, meterPercent, meterText, action, actionHref, onAction }: { index: number; icon: React.ReactNode; title: string; name: string; readiness: Readiness; meterLabel: string; meterValue?: number | null; meterPercent?: number; meterText?: string; action?: string; actionHref?: string; onAction?: () => void }) {
  ...
  return <article className={`ready-source-row tone-${readiness.tone} ${action ? "" : "no-action"}`}><span className="ready-step">{index}</span><span className="ready-source-icon">{icon}</span><div className="ready-source-name"><strong>{title}</strong><span title={name}>{name}</span></div><div className="ready-source-status"><span className="ready-status-icon">{readiness.tone === "ready" || readiness.tone === "waiting" ? <Check /> : <TriangleAlert />}</span><span><strong>{readiness.label}</strong><small>{readiness.detail}</small></span></div><div className="ready-meter-wrap"><span>{meterLabel}</span>{meterText ? <strong className="ready-meter-text">{meterText}</strong> : <div aria-label={meterLabel} aria-valuemax={100} aria-valuemin={0} aria-valuenow={Math.round(percent)} className="ready-meter" role="meter">{Array.from({ length: 18 }, (_, i) => <span className={i < Math.round((percent / 100) * 18) ? "is-active" : ""} key={i} />)}</div>}</div>{action && (actionHref ? <a className="ready-change-button" href={actionHref}>{action}</a> : <button className="ready-change-button" onClick={onAction}>{action}</button>)}</article>;
}
